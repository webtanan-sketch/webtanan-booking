...
## SEO و Rank Math در نسخه 1.4.3

- صفحات پزشک از permalink استاندارد CPT و آرشیو از `/doctors/` استفاده می‌کنند.
- تخصص‌ها در حالت permalink مسیر مستقل `/doctors/specialty/{slug}/` دارند؛ لینک‌های قدیمی `?specialty_id=` به مسیر تمیز هدایت می‌شوند.
- canonical، عنوان و توضیح آرشیو تخصص از فیلترهای رسمی Rank Math تولید می‌شود.
- صفحات حساب، پنل، پرداخت، صف انتظار و نظرسنجی `noindex,nofollow` هستند و canonical عمومی دریافت نمی‌کنند.
- schema پزشک فقط یک‌بار در `wp_head` تولید می‌شود؛ نسخه تکراری داخل template حذف شده است.
- کارت پزشک و ویجت‌های Elementor، URL تخصص را از REST دریافت می‌کنند و امتیاز ساختگی نمایش نمی‌دهند.
