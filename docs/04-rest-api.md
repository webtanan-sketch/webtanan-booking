...
از نسخه `1.3.0` این endpoint از «Virtual Slot Generator» استفاده می‌کند:

- ورودی تاریخ شمسی یا میلادی به Gregorian `YYYY-MM-DD` تبدیل می‌شود.
- weekday انگلیسی از تاریخ نرمال‌شده محاسبه می‌شود.
- برنامه کاری پزشک از `wp_saas_schedules` برای همان weekday خوانده می‌شود.
- اسلات‌ها در حافظه از روی `start_time`, `end_time`, و `slot_duration` ساخته می‌شوند.
- فقط رکوردهای همان پزشک و همان تاریخ از `wp_saas_appointments` خوانده می‌شوند و روی اسلات‌های مجازی اعمال می‌شوند.
- اسلات بدون appointment مسدودکننده با `status=available` برمی‌گردد.

وضعیت‌های عمومی خروجی:

- `available`
- `locked`
- `booked`

رکوردهای لغوشده، lock منقضی‌شده یا غیرمسدودکننده، اسلات را برای نمایش عمومی نمی‌بندند و دوباره قابل انتخاب دیده می‌شوند.
