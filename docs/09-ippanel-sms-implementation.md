...
- نام متغیر کد ورود از مسیر «تنظیمات ← پیامک ← نام متغیر کد ورود» قابل تنظیم است؛ مقدار پیش‌فرض `verifyotp` است.
- نام متغیر باید عیناً با placeholder پترن تاییدشده در IPPanel برابر باشد. payload برای OTP فقط همان کلید و کد عددی شش‌رقمی را در `params` می‌فرستد.
- پاسخ REST ارسال OTP فقط وقتی موفق است که provider پیام را پذیرفته باشد. در خطای provider، رکورد OTP حذف و خطای فارسی قابل پیگیری برگردانده می‌شود.
- پاسخ کامل provider در `wp_saas_sms_logs` نگهداری می‌شود؛ حالت تست پیامک واقعی ارسال نمی‌کند.

## Retry و payload در نسخه 1.4.5

- خطاهای موقت DNS، timeout، اتصال، HTTP 429 و 5xx برای پیامک‌های سیستمی غیر OTP حداکثر سه بار retry می‌شوند.
- اولین retry حدود دو دقیقه بعد با single event انجام می‌شود و کران ۱۵ دقیقه‌ای نیز خطاهای باقی‌مانده را بررسی می‌کند.
- قفل transient از اجرای همزمان retry جلوگیری می‌کند.
- OTP و پیامک معمولی مدیریت خودکار retry نمی‌شوند تا کد ورود یا متن آزاد دوبار به کاربر نرسد.
- متغیرهای هر `message_type` whitelist مستقل دارند؛ برای نمونه پیامک صف، متغیرهای refund یا survey را دریافت نمی‌کند.
